...
//    begin
//      code := 48 + Random(75);
//      while ((code in [58..64]) or (code in [91..96])) do    //Коды символов,которые не берем
//        code := 48 + Random(75);
      ...
      while not DataModule2.IBQuery1.Eof do
        begin
          ...
          jsonBuffer.AddPair('origin_address',
            ...
            VarToStr(DataModule2.IBQuery1.FieldByName('APARTMENT').Value)+ ' эт.)');
...
//            VarToStr(DataModule2.IBQuery1.FieldByName('FLOOR_TO').Value)+ ' эт.)');
...
begin
  ResultJSON := TJSONObject.Create;
  //Получаем статус заказа от водителя
...
begin
  ...
  jsonData3.AddPair('title','FCM message');
                                   //Второй параметр - токен(Firebase)
...
begin
  ...
  try
    ...
    IdSSL.SSLOptions.Method := sslvSSLv23; //Версия SSL
    IdHTTP.IOHandler := IdSSL;
    //Соединяем IDHTTP c IDSSL
    IdHTTP.Request.ContentType := 'application/json';
    IdHTTP.Request.CharSet := 'UTF-8';
    IdHTTP.Request.CustomHeaders.Values['Authorization'] := 'key=AIzaSyA_sM6ipUnv6yL30Wh-MH4nXZyyCoxmlPo'; //Ключ от проекта в Firebase
    //То, что отображается на телефоне
    body := 'Новый заказ';
    title := 'Новый заказ';
    token := tokenDevice;
    ...
    jsonData2.AddPair('vibrate','1');  //Тип вибрации
    jsonData2.AddPair('sound','default');

    jsonData1.AddPair('to',token);  //кому отправляем
    jsonData1.AddPair('notification',jsonData2);

    Data := TStringStream.Create(jsonData1.ToString,TEncoding.UTF8);
    Answer := IdHTTP.Post('https://fcm.googleapis.com/fcm/send', Data); //Отправляем сообщение на Firebase
    //Вывели ответ
    ShowMessage(Answer);
  finally
    IdHTTP.Free;
...
//Сюда приходят json, основная процедура
