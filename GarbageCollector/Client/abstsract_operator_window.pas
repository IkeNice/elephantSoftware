...
begin
    ...
    for  i := 0 to count - 1 do    begin
      if dm.TOrders.FieldByName('STATUS_ID').AsInteger in ignore_status  then
      begin
        dm.TOrders.Next;
        continue;
      end;
      ...
      begin
//          if  (dm.TOrders.FieldByName('STATUS_ID').AsInteger <> queue[j].get_status ) then
//          begin
...
begin
    ...
    for  i := 0 to count - 1 do    begin
      ...
      j:=0;
      while ( j < queue.Count) and ( queue[j].get_id  <> dm.Qdrivers.FieldByName('emp_ID').AsInteger )  do
         j := j+1;
      // if found order but status changed
      if (j < queue.Count) then
      begin
//          if  (dm.TDrivers.FieldByName('STATUS').AsInteger <> queue[j].get_status ) then
//          begin
